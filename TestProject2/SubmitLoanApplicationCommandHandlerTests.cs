...
namespace TestProject2
{
    public class SubmitLoanApplicationCommandHandlerTests
    {
        ...
        [Fact]
        public async Task ReturnPendingStatusWhenLoanCriteriaNotMetForNewApplication()
        {

            var dto = new SubmitLoanApplicationDto(null, 200000, 40000, "Employed", 650, "Rented", 25, "789 def Blvd", 200000, 700, 3);//DTI:21% and LTV:100%
        ...
        [Fact]
        public async Task ReturnApprovedStatusWhenLoanCriteriaMetApplication()
        {

            var dto = new SubmitLoanApplicationDto(null, 200000, 40000, "Employed", 750, "Rented", 25, "789 def Blvd", 300000, 700, 4);//DTI:21% and LTV:66.7%
            var command = new SubmitLoanApplicationCommand(dto, 4);

            var result = await _handler.Handle(command, CancellationToken.None);

            var newApp = await _context.LoanApplications.FindAsync(result.id);
            newApp.Should().NotBeNull();
            newApp.UserId.Should().Be(4);
            newApp.LoanStatus.Should().Be("Approved");
        }
    }
}
